                    ...
                    <div>
                      ...
                      >
                        <SelectTrigger id={`input-inventory-${index}`} className="mt-1">
                          <SelectValue placeholder={t("inputs.selectMaterial")} />
                        </SelectTrigger>
                        <SelectContent>
                          {inventory.map((item) => (
                            <SelectItem key={item.id} value={item.id.toString()}>
                              {item.material.name} ({t("inputs.available")}: {item.quantity} {item.material.unit})
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>{t("outputs.title")}</CardTitle>
                  <CardDescription>{t("outputs.description")}</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-6 space-y-4">
                    ...
                    <div>
                      ...
                      >
                        <SelectTrigger id={`output-material-${index}`} className="mt-1">
                          <SelectValue placeholder={t("outputs.selectProduct")} />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="flex items-end gap-2">
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          min="0"
                          step="0.001"
                          placeholder={t("outputs.quantityPlaceholder")}
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t("submit.note")}</p>
              </div>
              <div className="flex gap-2">
                <Button type="button" variant="outline" asChild>
                  <Link href="/dashboard/processes">{t("actions.cancel")}</Link>
                </Button>
